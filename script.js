        ...
        // 创建全局AudioContext实例，避免频繁创建和销毁
        let globalAudioContext = null;
        
        // 获取或创建全局AudioContext实例
        ...
        function createGameOverEffect(gameType, score) {
            ...
            const restartHandler = function(e) {
                console.log('点击事件触发，gameType:', gameType);
        ...
        class DanmakuGame {
            ...
            playEnemyDeathSound() {
                try {
                    const audioContext = this.audioContext || getAudioContext();
                    const oscillator = audioContext.createOscillator();
                    const gainNode = audioContext.createGain();
        ...
        window.addEventListener('DOMContentLoaded', () => {
            ...
            if (today === 0) {
                console.log('检测到星期日，准备播放音频');
                const audio = new Audio();
                audio.loop = true;
                const audioUrls = [
                    'https://cdn.jsdelivr.net/gh/qiuhaiBW/qiuhaiweb@主干/鸟叫.mp3',
                    'https://cdn.jsdelivr.net/gh/qiuhaiBW/qiuhaiweb@main/鸟叫.mp3',
                    'https://raw.githubusercontent.com/qiuhaiBW/qiuhaiweb/%E4%B8%BB%E5%B9%B2/鸟叫.mp3',
                    'https://raw.githubusercontent.com/qiuhaiBW/qiuhaiweb/main/鸟叫.mp3'
                ];
                ...
                function tryPlayAudio() {
                    if (audioIndex >= audioUrls.length) {
                        console.error('所有音频URL都加载失败');
                        alert('音频加载失败，请检查：\n1. 音频文件是否已上传到GitHub仓库\n2. 文件名是否正确（鸟叫.mp3）\n3. 分支名称是否正确（主干）');
                        return;
                    }
                    
                    const url = audioUrls[audioIndex];
                    console.log(`尝试加载音频 (${audioIndex + 1}/${audioUrls.length}):`, url);
                    
                    audio.onerror = function() {
                        console.error(`音频URL ${audioIndex + 1} 加载失败:`, url);
                        audioIndex++;
                        tryPlayAudio();
                    };
                    ...
                    audio.oncanplay = function() {
                        console.log('音频可以播放，当前状态:', audio.readyState);
                        audioPlayed = true;
                    };
                    
                    audio.onloadeddata = function() {
                        console.log('音频数据加载成功，等待用户交互');
                        audioPlayed = true;
                    };
                ...
                document.addEventListener('click', function playAudioOnClick() {
                    console.log('用户点击，尝试播放音频');
                    if (audioPlayed && audio.readyState >= 2) {
                        console.log('音频已准备好，开始播放');
                        audio.play().then(() => {
                            console.log('音频播放成功，循环播放已启用');
                            document.removeEventListener('click', playAudioOnClick);
                        }).catch(err => {
                            console.error('播放失败:', err);
                            alert('播放失败，请检查浏览器是否允许自动播放音频');
                        });
                    } else {
                        console.log('音频未准备好或已播放，audioPlayed:', audioPlayed, 'readyState:', audio.readyState);
                        if (!audioPlayed) {
                            alert('音频正在加载中，请稍后再试');
                        }
                    }
                ...
                console.log('检测到星期一，准备播放音频');
                const audio = new Audio();
                audio.loop = true;
                const audioUrls = [
                    'https://cdn.jsdelivr.net/gh/qiuhaiBW/qiuhaiweb@主干/拂晓.mp3',
                    'https://cdn.jsdelivr.net/gh/qiuhaiBW/qiuhaiweb@main/拂晓.mp3',
                    'https://raw.githubusercontent.com/qiuhaiBW/qiuhaiweb/%E4%B8%BB%E5%B9%B2/拂晓.mp3',
                    'https://raw.githubusercontent.com/qiuhaiBW/qiuhaiweb/main/拂晓.mp3'
                ];
            ...
            mainTitle.addEventListener('click', () => {
                // 尝试多个可能的URL
                const possibleUrls = [
                    'https://cdn.jsdelivr.net/gh/qiuhaiBW/qiuhaiweb@主干/11.jpg',
                    'https://cdn.jsdelivr.net/gh/qiuhaiBW/qiuhaiweb@main/11.jpg',
                    'https://raw.githubusercontent.com/qiuhaiBW/qiuhaiweb/%E4%B8%BB%E5%B9%B2/11.jpg',
                    'https://raw.githubusercontent.com/qiuhaiBW/qiuhaiweb/main/11.jpg'
                ];
                ...
                function tryLoadImage() {
                    ...
                    console.log(`尝试加载图片 (${currentIndex + 1}/${possibleUrls.length}):`, url);
                    
                    modalImage.onerror = function() {
                        console.error(`URL ${currentIndex + 1} 加载失败:`, url);
                        currentIndex++;
                        tryLoadImage();
                    };
        ...
        class TwentyFortyEightGame {
            ...
            updateDisplay() {
                ...
                // 移除2048游戏结束效果
                const gameOverEffect = document.getElementById('twentyFortyEightGameOverEffect');
                if (gameOverEffect) {
                    gameOverEffect.style.opacity = '0';
        ...
        document.getElementById('twentyFortyEightGameBtn').addEventListener('click', () => {
            ...
            if (!twentyFortyEightGame) {
                ...
                window.twentyFortyEightGame = twentyFortyEightGame;
            } else {
